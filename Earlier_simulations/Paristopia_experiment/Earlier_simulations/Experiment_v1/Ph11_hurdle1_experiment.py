...
INITIAL_PRICE = 1.50 #CHANGE TO WHATEVER
...
def numTripsinHour(t):
    ...
    if t >= 23 and t < 24:
        return 4540
    if t>=24 and t < 25:
        return 50 #######CHEATING CHEATING - THIS IS A TEST
    else:
...
def adjustStationPrice(station, isRentBike):
    ...
    if isRentBike == True:  #HIGH NUMBIKES RENTBIKE EXPONENTIAL
    ...
    else: #isRentBike == False #HIGH NUMBIKES DROPOFF BIKE EXPONENTIAL
        if station.num_bikes < 13:
            station.price = (1.5/13)*(station.num_bikes)
        elif station.num_bikes >= 13 and station.num_bikes < 18:
            station.price = (1.5/19)*(station.num_bikes)+(3-((1.5*32)/19))
        else: #num_bikes >= 18
            station.price = (1.0/28)*(station.num_bikes-18)+2.5
...
def visualizeCity(station_list,t):
    ...
    image_name = 'Paristopia_Experiment2_t='+str(t)+'.png'
    plt.savefig(image_name, bbox_inches='tight') #find a way to change the picture name
    #plt.show()
